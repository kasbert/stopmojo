...
public class Util {
	...
	/**
	 * Left justify s into a string len wide. Note: does not remove whitespace
	 * from s before justifying so it really just pads the end of s to make it
	 * len wide. This behavior, although wrong, is maintained for backwards
	 * compatibility.
	 * 
	 * @param s
	 *            string to be left justified
	 * @param len
	 *            width to left justify s into
	...
	public static String getTimestampDate(Timestamp ts) {
		...
		c.setTimeInMillis(ts.getTime());
		return formatNumber("00", c.get(Calendar.MONTH) + 1) + "/" + formatNumber("00", c.get(Calendar.DAY_OF_MONTH))
	...
	public static String getTimestampTime(Timestamp ts) {
		...
		c.setTimeInMillis(ts.getTime());
		return formatNumber("00", c.get(Calendar.HOUR_OF_DAY)) + formatNumber("00", c.get(Calendar.MINUTE));
	}
	...
	public static String Time_Add(String Time, double Hrs) {
		...
		return nf.format(Math.floor(TimeHours)) + nf.format(Math.floor((TimeHours - Math.floor(TimeHours)) * 60));
	}
	...
	public static double Time_Diff(String Tim1, String Tim2) {
		...
		T1 = atof(Tim1.substring(0, 1)) + (atof(Tim1.substring(2, 3)) / 60.0);
		T2 = atof(Tim2.substring(0, 1)) + (atof(Tim2.substring(2, 3)) / 60.0);
	...
	public static double Cal_Julian(String DateVar) {
		...
		return (Math
				.floor(Math.floor(Math.floor(367.0 * Yl) - Math.floor(Yl) - 0.75 * Math.floor(Yl) + DayV) - 0.75 * 2.0)
				+ 1721115.0);
	}
	...
	/*
	 * Returns in Buffer a date in the form YY/MM/DD from the original string
	 * Date in the form MM/DD/YY. This routine is useful for put dates in a
	 * format in which they can easily be compared using string comparisons.
	 * Undate_Comp can be used to convert the date back again. Y2K - if the YY
	 * is < 50, then the first digit becomes 'A' or 'B' or 'C' or 'D' (ie
	 * A1/05/04 is 2001/05/04 B5/04/05 is 2015/04/05).
	 * 
	 * Date may be the same as buffer - ie. Date_Comp(TStr, TStr);
	 */
	...
	private static String[] DOW = { "sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday" };
	...
	/*
	 * Returns the day of the week in *Buffer for the calendar date passed in
	 * *Date in the form MM/DD/YY. The following formats are selected by
	 * DayCase: DayCase = 1 - MON, TUE, WED, THU... DayCase = 2 - Mon, Tue, Wed,
	 * Thu... DayCase = 3 - mon, tue, wed, thu... DayCase = 4 - MONDAY, TUESDAY,
	 * WEDNESDAY.... DayCase = 5 - Monday, Tuesday, Wednesday.... DayCase = 6 -
	...
	private static String[] MOY = { "january", "february", "march", "april", "may", "june", "july", "august",
	...
	/*
	 * Returns the month of year for the parameter Date (MM/DD/YY) in the format
	 * specified by the MonthCase: MonthCase = 1 - JAN, FEB, MAR, APR...
	 * MonthCase = 2 - Jan, Feb, Mar, Apr... MonthCase = 3 - jan, feb, mar,
	 * apr... MonthCase = 4 - JANUARY, FEBRUARY, MARCH, APRIL.... MonthCase = 5
	 * - January, February, March, April.... MonthCase = 6 - january, february,
	 * march, april....
	 */
	{ /* Month_Of_Year */
		int MonthNo;
		...
		StringBuffer moy = new StringBuffer(MOY[MonthNo - 1]);
	 ...
	 * 
	 * MDYCase = 1 - January 18, 1987 MDYCase = 2 - Wednesday, September 18,
	 * 1987 MDYCase = 3 - Wed, September 18, 1987 MDYCase = 4 - Jan 18, 1987
	 * MDYCase = 5 - Wed, Jan 18, 1987 MDYCase = 6 - 18th day of January, 1987
	 */
	{ /* M_D_Y */
		String Sufx;
		...
		case 2:
			return Day_Of_Week(Date, 5) + " " + Month_Of_Year(Date, 5) + " " + nf.format(Day) + ", " + nf.format(Year);

		case 3:
			return Day_Of_Week(Date, 2) + " " + Month_Of_Year(Date, 5) + " " + nf.format(Day) + ", " + nf.format(Year);

		case 4:
			return Month_Of_Year(Date, 2) + " " + nf.format(Day) + ", " + nf.format(Year);

		case 5:
			return Day_Of_Week(Date, 2) + ", " + Month_Of_Year(Date, 2) + " " + nf.format(Day) + ", " + nf.format(Year);
	...
	/**
	 * Determine the number of digits, decimals, and ints in the specified rtype
	 * (Pattern).
	 * 
	 * @param Pattern
	 * @param patSize
	 ...
	 */
	public static int Num_Pattern_Size(String Pattern, PatternSize patSize) {
		int C;
	...
	public static String Pad_Date(String Date) {
		...
		return formatNumber("00", Month) + "/" + formatNumber("00", Day) + "/" + formatNumber("00", Year);
	}
	 ...
	 * 
	 * @param Ch
	 *            character to find position of
	 * @param St
	 *            string to scan for Ch
	 * @return offset (1 based) of the first occurence of Ch in St or 0 if not
	 *         found
	 */
	public static int Pos(char Ch, String St) {
		int i;
	 ...
	 * 
	 * @param Num
	 *            string containing number to count decimals in
	 * @return number of digits to the right of the decimal point or 0 if Num
	 *         has no decimal point
	 */
	public static int Num_Decs(String Num) { /* Num_Decs */
	 ...
	 *
	 * @param Num
	 *            string containing number in which to count
	 * @return count of the digits before the decimal point
	 */
	public static int Num_Ints(String Num) { /* Num_Ints */
		int Ints, C;
	...
	/**
	 * Reformat the number RlStr with the specified width and number of
	 * decimals.
	 * 
	 * @param RlStr
	 *            string containing number to be reformatted
	 * @param FldWdth
	 *            width of reformatted string/number
	 * @param NumDecs
	 *            number of digits after the decimal
	 * @return the reformatted string
	 */
	public static String Pattern(String RlStr, int FldWdth, int NumDecs) {
	 ...
	 *            character to count
	 * @param St
	 *            string in which to count occurences of Ch
	 * @return number of times Ch occurs in St
	 */
	public static int Num_Chars(char Ch, String St) { /* Num_Chars */
		int C, NumChs;
